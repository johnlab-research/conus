      ...
      "text/html" -> {
        ...
        s"<b>$modelName</b> <br> Contains $nbModels models each comprising $nbTimesteps timesteps" +
          table(cls := "table",style:=tableStyle)(
            ...
            for(SingleModelResults(id,nbSteps,theGrid,initialConditions,calibrated,modelName,rsme,timeTaken) <-allResults.results) yield(
              ...
              )).render} else {"<b>No results under this model name. Try again later to see if more results have been computed</b>"}
      }
    ).asJava
  })
